	...
	for (int y = 0; y < game->HEIGHT; ++y)
	{
		for (int x = 0; x < game->WIDTH; ++x)
		{
			if (Game::map[game->HEIGHT - 1 - y][x] == 1)
			{
				...
				Game::Curmap[y][x] = wall;
			}
			else
			{
				Game::Curmap[y][x] = nullptr;
			}
		}
	}
}
...
void GameManager::MakePlayer()
{
	...
	Game::Curmap[player1->GetCoordAsInt().getY()][player1->GetCoordAsInt().getX()] = player1;
	Game::Curmap[player1->GetCoordAsInt().getY() + 1][player1->GetCoordAsInt().getX()] = player1;
	Game::Curmap[player2->GetCoordAsInt().getY()][player2->GetCoordAsInt().getX()] = player2;
	Game::Curmap[player2->GetCoordAsInt().getY() + 1][player2->GetCoordAsInt().getX()] = player2;
}
...
void GameManager::MakeItem()
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (!Item)		// Item == 0 이면 무기
		{
			...
			Game::Curmap[weapon->GetCoordAsInt().getY()][weapon->GetCoordAsInt().getX()] = weapon;
		}
		...
		else			// Item == 1 이면 특수 아이템
		{
			...
			Game::Curmap[item->GetCoordAsInt().getY()][item->GetCoordAsInt().getX()] = item;
		}
	}
}
...
void GameManager::PlayerShoot(PlayerCharacter* player)
{
	...
	for (int i = 0; i < (player->isWeaponShotgun() ? 2 : 1); i++)
	{
		...
		{
			p->SetCoord(player->GetCoord() + Vec2<double>{ 1, i });
			p->SetNextCoord(player->GetCoord() + Vec2<double>{ 1, i });
			p->SetVelocity(Vec2<double>{ 1, 0 });
		}
		...
		{
			p->SetCoord(player->GetCoord() + Vec2<double>{ i, 2 });
			p->SetNextCoord(player->GetCoord() + Vec2<double>{ i, 2 });
			p->SetVelocity(Vec2<double>{ 0, 1 });
		}
		else
		{
			p->SetCoord(player->GetCoord() + Vec2<double>{ i, -1 });
			p->SetNextCoord(player->GetCoord() + Vec2<double>{ i, -1 });
			p->SetVelocity(Vec2<double>{ 0, -1 });
		}
...
Vec2<int> GameManager::SetItemCoord()
{
	random_device rd_variable;
	mt19937 generate(rd_variable());
	...
	Vec2<int> coord(XCoord(generate), YCoord(generate));

	if (Game::Curmap[coord.getY()][coord.getX()] == nullptr)
	{
		return coord;
	}
